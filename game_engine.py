...
class GameEngine:
    ...
    def __init__(self, messaging_api, database):
        ...
        self.db = database
        self.active_games = {}          # group_id -> BaseGame instance
        self.waiting_for_name = {}      # user_id -> bool
        self.user_states = {}           # user_id -> state
        self.games = self._load_all_games()
        logger.info(f"GameEngine initialized with {len(self.games)} games")
    ...
    def _load_game(self, module_name: str, class_name: str):
        try:
            ...
            logger.debug(f"Loaded game: {class_name}")
            return game_class
    ...
    def process_message(self, text: str, user_id: str, display_name: str, is_registered: bool, theme: str):
        text = text.strip()
        group_id = user_id  # يمكن تعديلها لدعم المجموعات

        if group_id in self.active_games:
            return self._handle_game_answer(text, user_id, group_id, display_name, theme)
        return self._handle_game_start(text, user_id, group_id, is_registered, theme)

    def _handle_game_start(self, text: str, user_id: str, group_id: str, is_registered: bool, theme: str):
        if text in self.games:
            if not is_registered and text != "توافق":
                return TextMessage(text="يجب التسجيل أولا\nاكتب: تسجيل")
    ...
    def _start_game(self, game_name: str, user_id: str, group_id: str, theme: str):
        ...
        try:
            ...
            logger.info(f"Game started: {game_name} for user {user_id}")
    ...
    def _start_text_game(self, game_name: str):
        ...
        try:
            ...
            return TextMessage(text="لا يوجد محتوى متاح")
    ...
    def _handle_game_answer(self, text: str, user_id: str, group_id: str, display_name: str, theme: str):
        ...
        try:
            ...
            if result is None:
                return None
            if hasattr(game, "normalize_result"):
                result = game.normalize_result(result)
            points = result.get("points", 0)
            if points > 0 and self.db:
            ...
            logger.error(f"Game error for user {user_id}: {e}", exc_info=True)
