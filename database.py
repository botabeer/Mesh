...
class Database:
    ...
    def get_user_stats(user_id: str) -> Optional[Dict]:
        try:
            with Database.get_connection() as conn:
                row = conn.execute("SELECT display_name, total_points, games_played, wins FROM users WHERE user_id = ?", (user_id,)).fetchone()
                if not row:
                    return None
                return dict(row)
        except Exception:
    ...
    def get_leaderboard(limit: int = 20) -> List[Tuple[str, int]]:
        ...
        try:
            with Database.get_connection() as conn:
                rows = conn.execute("SELECT display_name, total_points FROM users WHERE games_played > 0 ORDER BY total_points DESC, wins DESC LIMIT ?", (limit,)).fetchall()
    ...
    def get_user_rank(user_id: str) -> Optional[int]:
        try:
            with Database.get_connection() as conn:
                row = conn.execute("SELECT COUNT(*) + 1 FROM users WHERE total_points > (SELECT total_points FROM users WHERE user_id = ?)", (user_id,)).fetchone()
    ...
    def cleanup_inactive_users() -> int:
        ...
        with Database._lock:
            try:
                with Database.get_connection() as conn:
                    cur = conn.execute("DELETE FROM users WHERE last_activity < ?", (cutoff.isoformat(),))
