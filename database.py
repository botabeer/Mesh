...
def retry_on_locked(max_retries=5, delay=0.2):
    """محسّن: retry مع exponential backoff"""
    def decorator(func):
        @wraps(func)
...
class ConnectionPool:
    """محسّن: Connection pool مع health checks"""
    ...
    def record_game_stat(self, user_id: str, game_name: str, score: int, won: bool = False) -> bool:
        ...
        with self.pool.get_connection() as conn:
            ...
            cursor.execute('''
                ...
                (user_id, game_name, plays, wins, total_score, best_score, last_played) 
                VALUES (?, ?, 1, ?, ?, ?, ?) 
