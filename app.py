...
def is_rate_limited(user_id: str) -> bool:
    ...
    timestamps = [
        t for t in user_rate_limit[user_id]
        if now - t < window
    ]
...
# ===========================
# جلب ملف المستخدم مع التخزين المؤقت
# ===========================
...
h1 {{
    color: #667eea; font-size: 2.5em; margin-bottom: 10px; text-align: center;
}}
.version {{
...
.status {{
    ...
    border-radius: 25px; font-weight: bold; margin: 15px 0;
}}
.stats {{
    display: grid;
...
.footer {{
    ...
    border-top: 2px solid #eee; text-align: center; color: #999;
}}
</style>
</head>
<body>
...
<div class="stats">
    ...
    <div class="stat"><div class="stat-label">إجمالي الألعاب</div><div class="stat-value">{total}</div></div>
    ...
    <div class="stat"><div class="stat-label">الجلسات الكلية</div><div class="stat-value">{stats.get('total_sessions', 0)}</div></div>
        ...
        with ApiClient(configuration) as api_client:
            ...
            if text.startswith("ثيم "):
                ...
                if Config.is_valid_theme(new_theme):
                    ...
                    mode = user_sessions.get(ctx_id, {}).get("mode", "فردي")
                    msg = ui.home_screen(username, points, is_reg, new_theme, mode)
                else:
                    msg = TextMessage(
                        text="ثيم غير موجود\nالثيمات المتاحة:\nابيض - اسود - ازرق - بنفسجي - وردي - اخضر - رمادي - احمر - بني"
                    )
            ...
            # وضع الفريقين و الوضع الفردي (للقروبات فقط)
            if src_type in ["group", "room"]:
                if norm == "فريقين":
                    user_sessions.setdefault(ctx_id, {})["mode"] = "فريقين"
                    line_api.reply_message(
                        ReplyMessageRequest(reply_token=event.reply_token, messages=[TextMessage(text="تم تفعيل وضع الفريقين")])
                    )
                    return
                ...
                if norm == "فردي":
                    ...
                    line_api.reply_message(
                        ReplyMessageRequest(reply_token=event.reply_token, messages=[TextMessage(text="تم تفعيل الوضع الفردي")])
                    )
                    return
