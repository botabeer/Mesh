# Bot Mesh - LINE Bot with Arabic Normalization & Emoji Control
...
GAME_RULES = {
    ...
    "allowed_emojis": ["▫️","▪️","🏅","🏆"],  # قاعدة ثابتة لاستخدام الإيموجي
    "normalize_arabic": True  # قاعدة ثابتة لتوحيد الحروف العربية
}
...
def normalize_arabic(text):
    if not GAME_RULES["normalize_arabic"]:
        return text.strip().lower()
    text = text.strip().lower()
    # إزالة التشكيل
    text = re.sub(r'[\u0617-\u061A\u064B-\u0652]', '', text)
    # استبدال الحروف المشابهة
...
def verify_answer(question, answer):
    ...
    return norm_answer in norm_question
...
def winner_screen(game_name):
    ...
    message = f"{GAME_RULES['allowed_emojis'][3]} الفائز: {winner_text}\nالنقاط: {max_score}"
    bubble = {
        "type": "bubble","size":"mega",
        "body":{"type":"box","layout":"vertical","spacing":"md","contents":[
            {"type":"text","text":f"{GAME_RULES['allowed_emojis'][3]} انتهت اللعبة!","weight":"bold","size":"xl","align":"center"},
    ...
    if getattr(event,"type",None)=="message" and getattr(event.message,"type",None)=="text":
        ...
        with ApiClient(configuration) as api_client:
            ...
            if text=="انضم":
                ...
                line_bot_api.reply_message(ReplyMessageRequest(reply_token=event.reply_token,messages=[TextMessage(text="تم تسجيلك بنجاح!")]))
                return
            now=time.time()
            ...
            if GAME_RULES["registered_only"] and user_id not in USERS:
                line_bot_api.reply_message(ReplyMessageRequest(reply_token=event.reply_token,messages=[TextMessage(text="عليك التسجيل أولاً باستخدام 'انضم'")]))
                return
            if text=="بداية":
                line_bot_api.reply_message(ReplyMessageRequest(reply_token=event.reply_token,messages=[welcome_screen()]))
                return
            if text=="مساعدة":
                line_bot_api.reply_message(ReplyMessageRequest(reply_token=event.reply_token,messages=[games_menu()]))
                return
            if text=="انسحب":
                if user_id in USERS:
                    USERS[user_id]["active"]=False
                line_bot_api.reply_message(ReplyMessageRequest(reply_token=event.reply_token,messages=[TextMessage(text="تم الانسحاب ولن يتم احتساب إجاباتك لاحقًا.")]))
                return
            if text.startswith("توافق"):
                parts=text.split()
                if len(parts)==3:
                    perc=compatibility_percentage(parts[1],parts[2])
                    line_bot_api.reply_message(ReplyMessageRequest(reply_token=event.reply_token,messages=[TextMessage(text=f"نسبة التوافق بين {parts[1]} و {parts[2]}: {perc}%")]))
                    return
            if text=="إيقاف":
                CURRENT_GAMES.clear()
                line_bot_api.reply_message(ReplyMessageRequest(reply_token=event.reply_token,messages=[TextMessage(text="تم إيقاف جميع الألعاب.")]))
                return
            line_bot_api.reply_message(ReplyMessageRequest(reply_token=event.reply_token,messages=[TextMessage(text="تم استلام رسالتك")]))
