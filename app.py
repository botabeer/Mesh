...
from ui_builder import (build_games_menu,build_my_points,build_leaderboard,build_registration_status,
                        build_winner_announcement,build_help_window,build_enhanced_home,
...
def validate_username(username:str)->tuple[bool,str]:
    ...
    if len(username)>100:
        return False,"الاسم طويل جدا - 100 حرف كحد اقصى"
...
try:
    ...
    AVAILABLE_GAMES={
        "ذكاء":IqGame,"روليت":RouletteGame,"لون":WordColorGame,"ترتيب":ScrambleWordGame,
        "أسرع":FastTypingGame,"ضد":OppositeGame,"تكوين":LettersWordsGame,"أغنيه":SongGame,
        "لعبة":HumanAnimalPlantGame,"سلسلة":ChainWordsGame,"خمن":GuessGame,"توافق":CompatibilitySystem,
...
def ensure_session_meta(game_id):
    if game_id not in session_meta:
        session_meta[game_id]={"session_id":None,"team_mode":False,"current_game_name":None,
                              "session_type":"solo","start_time":time.time()}
...
def launch_game_instance(game_id,owner_id,game_class_name,line_api,theme=None,team_mode=False,source_type="user"):
    ...
    if game_class_name not in ["توافق","مافيا"]:
        session_id=db.create_game_session(owner_id,game_class_name,mode=game_instance.session_type,
...
def health_check():
    try:
        return jsonify({"status":"healthy","timestamp":datetime.now().isoformat(),
                       "active_games":len(active_games),"cached_users":len(user_cache.cache)}),200
    except Exception as e:
        ...
        with ApiClient(configuration) as api_client:
            ...
            if user_id in pending_registrations:
                ...
                if is_valid:
                    ...
                    response=TextMessage(text=f"خطا: {result}\nحاول مرة اخرى")
                    line_api.reply_message(ReplyMessageRequest(reply_token=event.reply_token,messages=[response]))
                return
            ...
            if normalized in ["انضم","join","تسجيل"]:
                if is_registered:
                    response=TextMessage(text=f"انت مسجل مسبقا\nالاسم: {username}\nالنقاط: {points}")
                else:
            ...
            if normalized in ["انسحب","leave","خروج"]:
                ...
                line_api.reply_message(ReplyMessageRequest(reply_token=event.reply_token,messages=[response]))
                return
            if normalized in ["إيقاف","ايقاف","stop"]:
                if game_id in active_games:
                    game_name=active_games[game_id].game_name
                    del active_games[game_id]
                    if game_id in session_meta:
                        del session_meta[game_id]
                    response=build_game_stopped(game_name,theme)
                    line_api.reply_message(ReplyMessageRequest(reply_token=event.reply_token,messages=[response]))
                return
            ...
            if game_class_name in AVAILABLE_GAMES:
                if game_class_name=="توافق":
                    ...
                    return
                if game_class_name=="مافيا":
                    if source_type not in ["group","room"]:
                        response=TextMessage(text="لعبة المافيا للمجموعات فقط")
                        line_api.reply_message(ReplyMessageRequest(reply_token=event.reply_token,messages=[response]))
                        return
                    game=launch_game_instance(game_id,user_id,game_class_name,line_api,theme,False,source_type)
                    question_msg=game.start_game()
